...
<<<<<<< HEAD
        if (source.equals("assetreport1")) {
            ...
            try (Workbook workbook = new XSSFWorkbook()) {
                ...
                // Save the workbook to a file
                try (FileOutputStream fileOut = new FileOutputStream("C:/Users/Afiq/OneDrive/Documents/NetBeansProjects/ITAM/web/assets/reports/output1.xlsx")) {
                    workbook.write(fileOut);
                }
            ...
            try(Workbook workbook = new XSSFWorkbook())
            {
                ...
                Cell mergedCell = headerRow.getCell(0);
                mergedCell.setCellStyle(style);
                ...
                {
                    Row tableRow = sheet.createRow(i+3);
...
<<<<<<< HEAD
                    //ada error dekat sini
                    for(int m=0;m<header.length;m++)
                    {
                        cell = tableRow.createCell(m);
                        cell.setCellValue(data[m]); 
                        
                        Cell tablecell = tableRow.getCell(m);
                        tablecell.setCellStyle(styleborder);
                    }
...
<<<<<<< HEAD
        else if(source.equals("assetreport4"))
        {
            String filePath = "C:\\Users\\Afiq\\OneDrive\\Documents\\NetBeansProjects\\ITAM\\web\\assets\\reports\\templateAsset.xlsx";
            try 
            {
            ...
            // Find the table by its displayName (table name)
            List<XSSFTable> tables = sheet.getTables(); // Replace "YourTableName" with the actual table name
            ...
            for (XSSFTable table : tables)
            {
                if (table != null) {
                    ...
                    // Create a new row and append data to it
                    
                    
                    /*Row newRow = sheet.createRow(endCell.getRow());
                    Cell cell1 = newRow.createCell(startCell.getCol());
                    cell1.setCellValue("New Data 1");
                    Cell cell2 = newRow.createCell(startCell.getCol() + 1);
                    ...
                    */
                    ...
                    for (int i = 0; i < listAssets.size(); i++) {
                        ...
                        Row newRow = sheet.createRow(endCell.getRow()+i);
                        ...
                        for (int m = 0; m < data.length; m++) 
                        {
                            ...
                            Cell cell1 = newRow.createCell(startCell.getCol() + m);
                            cell1.setCellValue(data[m]);
                            num = cell1.getColumnIndex();
                        
                        }
                         
                        //endCell = new CellReference(newRow.getRowNum(),);
                        //endCell = table.getEndCellReference();
                    }

                    // Write the changes back to the Excel file
                    String fileOut = "C:/Users/Afiq/OneDrive/Documents/NetBeansProjects/ITAM/web/assets/reports/output3.xlsx";
                    FileOutputStream fileOutputStream = new FileOutputStream(fileOut);
                    workbook.write(fileOutputStream);
                    fileOutputStream.close();
                    ...
                    System.out.println("Table not found in the sheet.");
                }
            }
